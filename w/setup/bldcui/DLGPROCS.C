...
**    Text       - IDC_TEXT1 through IDC_TEXT14
**
** Initialization Symbols:
   ...
   switch (wMsg)
   {
      ...
      case WM_INITDIALOG:
         ...
         for (i = 1; i <= cItems; ++i)
         {
            ...
            Assert(cb < cbSymBuf);
            CheckDlgButton(hdlg, idc++,
                           (lstrcmp(szSymBuf, "ON") == 0) ? TRUE : FALSE );
         }
      ...
      case WM_COMMAND:
         ...
         for (idc = IDC_B1; GetDlgItem(hdlg, idc); idc++)
         {
            //MessageBox( 0, szEvent, "DGC", MB_OK );
...
/*
** Purpose:
**    Edit Dialog procedure for templates with one to ten Edit controls.
**    (Limits the input string length to cbFullPathMax characters.)
**
** Controls Recognized:
**    Edit       - IDC_EDIT, IDC_EDIT2, IDC_EDIT3, IDC_EDIT4, IDC_EDIT5,
...
BOOL FAR PASCAL FEditDlgProc(HWND hdlg, WORD wMsg, WORD wParam, LONG lParam)
{
   ...
   switch (wMsg)
   {
      ...
      case WM_INITDIALOG:
         ...
         cb = CbGetSymbolValue("EditTextIn2", szSymBuf, cbFullPathMax + 1);
         Assert(cb < cbFullPathMax + 1);
         SendDlgItemMessage(hdlg, IDC_EDIT2, EM_LIMITTEXT, cbFullPathMax, 0L);
         SetDlgItemText(hdlg, IDC_EDIT2, (LPSTR)szSymBuf);

         cb = CbGetSymbolValue("EditTextIn3", szSymBuf, cbFullPathMax + 1);
         Assert(cb < cbFullPathMax + 1);
         SendDlgItemMessage(hdlg, IDC_EDIT3, EM_LIMITTEXT, cbFullPathMax, 0L);
         SetDlgItemText(hdlg, IDC_EDIT3, (LPSTR)szSymBuf);
         ...
         switch(wParam)
         {
            ...
            case IDCANCEL:
               ...
               SendDlgItemMessage(hdlg, IDC_EDIT2, (WORD)WM_GETTEXT,
                                  cbFullPathMax + 1, (LONG)((LPSTR)rgchText));
               if (!FSetSymbolValue("EditTextOut2", rgchText))
               {
                  DestroyWindow(GetParent(hdlg));
                  return(fTrue);
               }
               SendDlgItemMessage(hdlg, IDC_EDIT3, (WORD)WM_GETTEXT,
                                  cbFullPathMax + 1, (LONG)((LPSTR)rgchText));
               if (!FSetSymbolValue("EditTextOut3", rgchText))
               {
                  DestroyWindow(GetParent(hdlg));
                  return(fTrue);
               }
...
BOOL FAR PASCAL FRadioDlgProc(HWND hdlg, WORD wMsg, WORD wParam, LONG lParam)
{
   ...
   switch (wMsg)
   {
      ...
      case WM_INITDIALOG:
         cb = CbGetSymbolValue("EditTextRI", szSymBuf, cbFullPathMax + 1);
         Assert(cb < cbFullPathMax + 1);
         SendDlgItemMessage(hdlg, IDC_EDIT, EM_LIMITTEXT, cbFullPathMax, 0L);
         SetDlgItemText(hdlg, IDC_EDIT, (LPSTR)szSymBuf);
      ...
      case WM_COMMAND:
         switch (wParam)
         {
            ...
            case IDCANCEL:
               ...
               SendDlgItemMessage(hdlg, IDC_EDIT, (WORD)WM_GETTEXT,
                                  cbFullPathMax + 1, (LONG)((LPSTR)rgchText));
               if (!FSetSymbolValue("EditTextRO", rgchText))
               {
                  DestroyWindow(GetParent(hdlg));
                  return(fTrue);
               }
...
/*
** Purpose:
**    Confirm Info Dialog procedure for templates with one to ten
