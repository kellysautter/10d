                            ...
                            zSHORT nShowMenuItemForUpdate )
{
   ...
   //BL: if Entity selected, then enable menu items for add Attribute and Entity.
...
zwTZZOLODD_OpenLOD( zVIEW vSubtask )
{
   ...
   if ( nRC < 1 )
   {
      strcpy_s( szMsg, zsizeof( szMsg ), "OpenLOD unable to access LOD file list. Open aborted." );
...
zwTZZOLODD_OkNewLOD( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szLOD_Type, zsizeof( szLOD_Type ), vProfileXFER, "ZO", "LOD_Type" );
...
zwTZZOLODD_SaveLOD( zVIEW vSubtask )
{
   ...
   if ( nRC >= zCURSOR_SET )
   {
	  ...
	  nRC = IncludeSubobjectFromSubobject( vLOD, "TE_DB_Environ",
      ...
      if ( nRC < zCURSOR_SET )
      {
         ...
         if ( bError )
         {
            ...
            strcpy_s( szMsg, zsizeof( szMsg ), "The LOD has not yet been saved \n" );
            strcat_s( szMsg, zsizeof( szMsg ), "to an existing TE Data Source." );
      ...
      nRC = SetCursorFirstEntityByAttr( vLOD, "TE_DBMS_Source", "ZKey",
                                        vLOD, "POD", "TE_SourceZKey",
                                        "TE_DB_Environ" );
	  // KJS 01/13/20 - SetCursorFirstEntityByAttr doesn't seem to work. Adding below SetCursor
	  GetIntegerFromAttribute(&lSourceZKey, vLOD, "POD", "TE_SourceZKey");
	  nRC = SetCursorFirstEntityByInteger(vLOD, "TE_DBMS_Source", "ZKey", lSourceZKey, "POD");
	  GetStringFromAttribute( szTE_Name, zsizeof( szTE_Name ), vLOD, "TE_DBMS_Source", "Name");
      strcat( szMsg, szTE_Name );
      MB_SetMessage( vSubtask, 1, szMsg );
   }
   else
   ...
   if ( nRC < 0 )
   {
      ...
      if ( nRC < 0 )
      {
         strcpy_s( szMsg, zsizeof( szMsg ), "Unable to Save VOR to LPLR." );
...
zwTZZOLODD_OK_NewEntity( zVIEW vSubtask )
{
   ...
   // Reset Indent Name in the case of a renamed Entity
   nRC = GetStringFromAttribute( szEntityName, zsizeof( szEntityName ),
                                 vTZZOLODO, "LOD_Entity", "Name" );
   ...
   // Reset Ident Name
   nRC = GetStringFromAttribute( szEntityNameIndent, zsizeof( szEntityNameIndent ),
...
zwTZZOLODD_PasteAttributes( zVIEW vSubtask )
{
         ...
         nRC = SetCursorNextEntity( vWork, "LOD_AttributeCopy", 0 ) )
   {
      ...
      if ( ActivateMetaOI_ByZKey( vSubtask, &vDomain, 0, zREFER_DOMAIN_META, zSINGLE, lZKey, 0 ) < 0 )
      {
         GetStringFromAttribute( szTemp, zsizeof( szTemp ), vWork, "DomainCopy", "Name" );
         ActivateMetaOI_ByName( vSubtask, &vDomain, 0, zREFER_DOMAIN_META, zSINGLE, szTemp, 0 );
      }
...
zwTZZOLODD_AcceptLOD_Detail( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szDoNotMergeOperationsFlag, zsizeof( szDoNotMergeOperationsFlag ), vTZZOLODO, "LOD", "DoNotMergeOperationsFlag" );
   GetStringFromAttribute( szDoNotMergeFlag, zsizeof( szDoNotMergeFlag ), vTZZOLODO, "LOD", "DoNotMergeFlag" );
...
zwTZZOLODD_SetSaveAsName( zVIEW vSubtask )
{
         ...
         nRC = SetCursorNextEntity( vSaveAs, "SourceFile", "" ) )
   {
      ...
      {
         nIndex = zwfnTZZOLODD_SaveAsSetSourceName( vSubtask, vSaveAs, vLodData, nIndex );
      }
                                  ...
                                  zVIEW    vSaveAs )
{
         ...
         nRC = SetCursorNextEntity( vSaveAs, "SourceFile", "" ) )
   {
      ...
      if ( SetCursorNextEntityByString( vSaveAsCopy, "SourceFile", "SaveAsName", szNewName, "" ) >= zCURSOR_SET )
      {
                              ...
                              zVIEW    vSaveAs )
{
         ...
         nRC = SetCursorNextEntity( vSaveAs, "SourceFile", "" ) )
   {
      ...
      {
         zwfnTZZOLODD_SaveAsGetFileName( vTaskLPLR, vSaveAs, "SourceName", szSourceFileName );
         zwfnTZZOLODD_SaveAsGetFileName( vTaskLPLR, vSaveAs, "SaveAsName", szNewFileName );
      ...
      if ( SetCursorFirstEntityByAttr( vSourceFile, "SourceFile", "Name",
                                       vSaveAs, "SourceFile", "SourceName", "" ) == zCURSOR_SET )
      {
...
zwTZZOLODD_SaveAsLOD( zVIEW vSubtask )
{
   ...
   //Name is required
   if ( zwfnTZZOLODD_SaveAsCheckName( vSubtask, vSaveAs, vTZZOLODO, szOutName ) < 0 )
   {
      return( -1 );
   }
   TraceLineS( "*** Trace 2", "" );
   ...
   // Name already exists
   if ( SetCursorFirstEntityByString( vTZZOLFLO, "W_MetaDef", "Name", szOutName, 0 ) > zCURSOR_UNCHANGED )
   {
                                    ...
                                    zVIEW   vOldLOD )
{
   ...
   if ( CheckExistenceOfEntity( vOldLOD, "LOD_ConstraintOper" ) >= zCURSOR_SET &&
        SetCursorFirstEntityByAttr( vNewLOD, "Operation", "Name",
                                    ...
                                    "LOD" ) >= zCURSOR_SET )
   {
      IncludeSubobjectFromSubobject( vNewLOD, "LOD_ConstraintOper", vNewLOD, "Operation", zPOS_AFTER );
   }
   ...
   while ( nRC > zCURSOR_UNCHANGED )
   {
      if ( CheckExistenceOfEntity( vOldLOD, "LOD_EntityConstraintOper" ) >= zCURSOR_SET &&
           SetCursorFirstEntityByAttr( vNewLOD, "LOD_Entity", "Name",
                                       vOldLOD, "LOD_Entity", "Name", "LOD" ) >= zCURSOR_SET )
      {
         SetCursorFirstEntityByAttr( vNewLOD, "Operation", "Name",
                                ...
                                zVIEW   vOldLOD )
{
   ...
   while ( nRC > zCURSOR_UNCHANGED )
   {
      ...
      SetMatchingAttributesByName( vNewLOD, "SourceFile", vOldLOD, "SourceFile", zSET_NULL );
...
zwTZZOLODD_AddParentEntity( zVIEW vSubtask )
{
   ...
   // Activate an empty LOD Object
   nRC = ActivateEmptyMetaOI( vSubtask, &vLOD_Tgt, zSOURCE_LOD_META, zSINGLE | zLEVEL_APPLICATION );
   if ( nRC < 0 )
   {
   ...
   SetMatchingAttributesByName( vLOD_Tgt, "LOD", vLOD_Src, "LOD", zSET_ALL );
   SetNameForView( vLOD_Tgt, "TZZOLODO", vSubtask, zLEVEL_TASK );
   nRC = CreateMetaEntity( vSubtask, vLOD_Tgt, "LOD_EntityParent", zPOS_AFTER );
   SetAttributeFromString( vLOD_Tgt, "LOD_EntityParent", "Name", "NewParentWork" );
   SetAttributeFromString( vLOD_Tgt, "LOD_EntityParent", "Desc", "NewParent Description" );
   ...
   nRC = IncludeSubobjectFromSubobject( vLOD_Tgt, "LOD_Entity", vLOD_Tgt, "LOD_EntityParent", zPOS_AFTER );
   ...
   RetrieveViewForMetaList( vSubtask, &vLPLR, zREFER_ERD_META );
   ActivateMetaOI( vSubtask, &vERD, vLPLR, zSOURCE_ERD_META, zSINGLE | zLEVEL_APPLICATION );
   for ( nRC = SetCursorFirstEntity( vLOD_Src, "LOD_EntityParent", 0 );
         nRC >= zCURSOR_SET;
         nRC = SetCursorNextEntity( vLOD_Src, "LOD_EntityParent", 0 ) )
   {
      oTZZOLODO_LOD_EntityCopy( vLOD_Tgt, vLOD_TgtRecur, vLOD_Src, vLOD_SrcRecur,
   ...
   if ( CheckExistenceOfEntity( vLOD_Src, "POD" ) >= zCURSOR_SET )
   {
      ...
      SetMatchingAttributesByName( vLOD_Tgt, "POD", vLOD_Src, "POD", zSET_NULL );
      IncludeSubobjectFromSubobject( vLOD_Tgt, "TE_DB_Environ", vLOD_Src, "TE_DB_Environ", zPOS_AFTER );
   }
...
// RetrieveViewForMetaList( vSubtask, &vLODList, zSOURCE_LOD_META );
   ...
   {
      k++;
      if ( ActivateMetaOI( vSubtask, &vLOD, vLODList, zSOURCE_LOD_META, zSINGLE | zLEVEL_TASK ) == 1 )
      {
         ...
         if ( lCheckFlags & zCHECK_TIMESTAMP_ONLY )
         {
            ...
            sprintf_s( szMsg, zsizeof( szMsg ), "Checking LOD: %s for synchronization", pch );
            TraceLineS( szMsg, "" );
            MB_SetMessage( vSubtask, 1, szMsg );
            ...
            strcat_s( szFileSpec2 + nLth, zsizeof( szFileSpec2 ) - nLth, ".xod" );
            ...
            strcpy_s( szFileSpec1 + nLth, zsizeof( szFileSpec1 ) - nLth, ".lod" );
                 ...
                 SysCopyFile( vSubtask, szFileSpec1, szFileSpec2, TRUE ) > 0 )
            {
               ...
               strcpy_s( szFileSpec2 + nLth, zsizeof( szFileSpec2 ) - nLth, ".lo~" );
         ...
         if ( bSaveAll )
         {
            ...
            sprintf_s( szMsg, zsizeof( szMsg ), "Saving LOD: %s", pch );
            MB_SetMessage( vSubtask, 1, szMsg );
            CommitMetaOI( vSubtask, vLOD, zSOURCE_LOD_META );
         }
      ...
      {
         TraceLineI( "MigrateToolLODsForOK unexpected failure to open LOD in list ", k );
      }
...
zwfnTZZOLODD_CheckActionAfterSaveAs( zVIEW  vSubtask )
{
   ...
   switch ( lActionAfterSaveAs )
   {
      case zNewComponentAfterSaveAs:   // Create new LOD
         SetWindowActionBehavior( vSubtask, zWAB_ReplaceWindowWithModalWindow, "TZZOLODD", "TZZONWLD" );
         break;

      case zOpenComponentAfterSaveAs:   // Open LOD
         SetWindowActionBehavior( vSubtask, zWAB_ReplaceWindowWithModalWindow, "TZZOLODD", "TZZOOPLD" );
         break;
...
zwTZZOLODD_CompareLODs( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szLOD_Name, zsizeof( szLOD_Name ), vTargetLOD, "LOD", "Name" );
...
zwTZZOLODD_SelectCompEntries( zVIEW vSubtask )
{
         ...
         nRC = SetCursorNextEntity( vTargetLOD, "DisplayCompareResult", "" ) )
   {
      ...
      if ( nRC == 1 )
      {
         ...
         GetStringFromAttribute( szType, zsizeof( szType ), vTargetLOD, "DisplayCompareResult", "EntityAttributeType" );
         if ( szType[ 0 ] == 'A' )
         {
            // Type is Attribute. Give error is this is a Target Attribute without a Source Attribute.
            GetStringFromAttribute( szMetaName, zsizeof( szMetaName ), vTargetLOD, "DisplayCompareResult", "SourceAttributeName" );
            if ( szMetaName[ 0 ] == 0 )
         ...
         else
         {
            ...
            GetStringFromAttribute( szMetaName, zsizeof( szMetaName ), vTargetLOD, "DisplayCompareResult", "SourceEntityName" );
            if ( szMetaName[ 0 ] == 0 )
               nErrorFlag = 1;     // Indicate there's been a selection error.
            else
               SetAttributeFromString( vTargetLOD, "DisplayCompareResult", "SelectedFlag", "Y" );
