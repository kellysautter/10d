...
/* Change log most recent first order:
   ...
   2002.07.26  DKS   2000   ActiveX
      Altered CheckBox mapping to use the CheckBox value rather than
      the text value.
   ...
   2001.7.3   TMV   2000   ActiveX
      implement handler for lose focus
...
short ZGridColumn::UpdateAttribute( zVIEW vApp, BOOL bValidateOnly )
{
   ...
   else if ( col.GetDataChanged() )// this for backward compatibility
   {
      ...
      if ( pMapping )
      {
         ...
         short nCallFlags = zACCEPT_NULL_ENTITY;

         if ( !(*pMapping->m_csContext) )
            nCallFlags |= zUSE_DEFAULT_CONTEXT;

         if ( bValidateOnly )
            nCallFlags |= zVALIDATE;
...
CString ZGridColumn::GetAttributeValue( zVIEW vApp )
{
   ...
   else // for backward compatibility
   {
      if( m_pMapping )
      {
         strEntity    = m_pMapping->m_csEName;
         strAttr      = m_pMapping->m_csAName;
         strContext   = m_pMapping->m_csContext;
...
zLONG ZGridColumn::PopulateListOfValues()
{
   ...
   if ( bListDomainOnly )
   {
      ...
      zSHORT nDomainType = tgGetDomainType( vApp, strEntity, strAttr );
      if ( nDomainType == 'T' )
         ListOfValues.Clear();// we do not like any user defined stuff here
      else
      {
      ...
      zSHORT nRC = GetFirstTableEntryForAttribute( pchBuffer, ulDisplayLth, vApp, strEntity, strAttr, strContext, &lCursor );
      while ( nRC == 0 )
      {
         ...
         nRC = GetNextTableEntryForAttribute( pchBuffer, ulDisplayLth, vApp, strEntity, strAttr, strContext, &lCursor );
      }
   ...
   if ( ulFormatType & zCOMBOBOX_POSITION_ONLY )
   {
      ...
      if ( strListContext.IsEmpty( ) )
      {
         ...
         if ( lpViewEntity )
         {
            ...
            {
               if ( zstrcmp( lpViewAttrib->szName, strListAttr.GetString() ) == 0 )
               {
...
BOOL ZGridColumn::CheckLocking( zVIEW vApp )
{
   ...
   {
      if( CheckExistenceOfEntity( vApp, pMapping->m_csEName ) == zCURSOR_SET )
...
void ZGridColumn::SetInitiallyLocked( BOOL bLockIt )
{
   ...
   if ( !bLockIt ) // if it's going to get unlocked check if Zeidon allows update
   {
      ...
      {
         zVIEW vApp = 0;
         GetViewByName( &vApp, pMapping->m_csVName,
                          m_pParentGrid->GetDialogView(), zLEVEL_ANY );
         if( vApp )
         {
            bColumnEditable = MiGetUpdateForViewEntityAttr( vApp,
