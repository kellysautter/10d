...
{
   m_cmRange.bColumnSel = ( bVal != VARIANT_FALSE );
   return S_OK;
}

HRESULT STDMETHODCALLTYPE CRangeX::get_ColumnSel(VARIANT_BOOL *pbVal)
{
   *pbVal = m_cmRange.bColumnSel ? VARIANT_TRUE : VARIANT_FALSE;
   return S_OK;
}

HRESULT STDMETHODCALLTYPE CRangeX::IsEmpty( VARIANT_BOOL __RPC_FAR *pbAnswer )
{
   BOOL bAnswer = ( m_cmRange.posStart.nCol == m_cmRange.posEnd.nCol &&
      m_cmRange.posStart.nLine == m_cmRange.posEnd.nLine );
   *pbAnswer = (VARIANT_BOOL)(bAnswer ? VARIANT_TRUE : VARIANT_FALSE);
   return S_OK;
}
