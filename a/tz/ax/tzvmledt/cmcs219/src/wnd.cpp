                     ...
                     HBRUSH hbrBackground, HICON hIcon )
{
   ...
   wsprintf( szClass, _T("CodeSense:%x:%x:%x:%x:%x"), (UINT)hInstance,
...
BOOL CWnd::SubclassWindow( HWND hWnd )
{
   ...
   BOOL bResult = ( m_OldWndProc != NULL );

#ifdef _ACTIVEX

   if( bResult )
      SetFontDisp( (HFONT)SendMessage( WM_GETFONT ) );
