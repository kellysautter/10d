...
// {A29D57AD-8492-47AD-A6C1-9B68677B182F}
const IID IID_ICodeSenseEvents = {
   0xa29d57ad,0x8492,0x47ad,{0xa6,0xc1,0x9b,0x68,0x67,0x7b,0x18,0x2f}
};
...
void CEditX::RaiseLastErrorException() const
{
   ...
   if (SUCCEEDED(hr))
   {
      pICreateErr->SetSource(L"CodeSense");
      wchar_t wszMsg[ 1024 ];
      LPTSTR pszMsg;

      FormatMessage( FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM,
                     NULL, GetLastError(), 0, &pszMsg, 0, NULL );
#ifdef _UNICODE
...
BOOL CEditX::ControlSiteWantsAccelerator(LPMSG pmsg)
{
   ...
   // give the control site a crack at the keystroke
   BOOL bHandled = FALSE;
...
HRESULT CEditX::TranslateAccelerator(LPMSG pmsg)
{
   ...
   // if the StealKeys property is enabled, don't give the control site
   // a crack at the message.  m_bStealKeys is set by the automation client
   // to lobotomize unruly containers.
   if (!m_bStealKeys)
      {
      switch (pmsg->wParam)
         {
         ...
         case VK_NEXT:
            {
               ...
               {
               case WM_KEYDOWN:
                  bHandled = ( 0 == m_pEdit->OnKeyDown( pmsg->wParam,
                     pmsg->lParam ) );
                  break;

               case WM_KEYUP:
                  bHandled = ( 0 == m_pEdit->OnKeyUp( pmsg->wParam,
                     pmsg->lParam ) );
                  break;

               case WM_SYSKEYDOWN:
                  bHandled = ( 0 == m_pEdit->OnSysKeyDown( pmsg->wParam,
                     pmsg->lParam ) );
                  break;
               }
            }
      ...
      if (!bHandled)
         bHandled = ControlSiteWantsAccelerator(pmsg);
      }

   if (!bHandled)
      bHandled = ::TranslateAccelerator(m_hWnd, g_hAccel, pmsg);
...
HRESULT CEditX::IOleObject_SetExtent(DWORD dwDrawAspect, SIZEL *psizel)
{
   ...
   // delphi 6 throws an exception if IOleObject_SetExtent() calls back
   // into the client site to notify the site of the change.  We need
   // to just eat the exception silently.  This only happens if a control
   // is created on the fly at form creation time.
   try
      {
      hr = CComControlBase::IOleObject_SetExtent(dwDrawAspect, psizel);
      }
   catch (...)
      {
      hr = E_FAIL;
...
void CEditX::GetEventNotificationParams(int nEvent, LPNMHDR pnmhdr, VARIANTARG *pParams, int &nDispID, int &nArgs, VARIANT& varResult) const
{
   ...
   switch(nEvent)
   {
      ...
      case CMN_FINDWRAPPED:
      {
         ...
         pParams[ nArgs++ ].boolVal = ( pfwd->bForward ? VARIANT_TRUE : VARIANT_FALSE );
      ...
      case CMN_CODETIPINITIALIZE:
      {
         ...
         case CMN_CODETIPUPDATE:
            ...
            break;
         default://CMN_CODETIPINITIALIZE
            nDispID = DISPID_NTFY_CODETIPINITIALIZE;
            break;
         }
      ...
      case CMN_CODELISTSELWORD:
      {
         nDispID = DISPID_NTFY_CODELISTSELWORD;

         LPCM_CODELISTSELWORDDATA pild = (LPCM_CODELISTSELWORDDATA)pnmhdr;
         ...
         VariantInit(pParams + nArgs);
         pParams[ nArgs ].vt = VT_I4;
         pParams[ nArgs++ ].lVal = (LONG) pild->iItem;
      ...
      case CMN_CODELISTSELCHANGE:
      {
         nDispID = DISPID_NTFY_CODELISTSELCHANGE;
         LPCM_CODELISTSELCHANGEDATA pild = (LPCM_CODELISTSELCHANGEDATA)pnmhdr;

         CDispatchWnd *pWnd = (CDispatchWnd*)CWnd::FromHandle(pild->clData.hListCtrl);
         ASSERT(NULL != pWnd);

         // push on BSTR return value
         varResult.vt = VT_BSTR;
         varResult.bstrVal = NULL;
         VariantInit(pParams + nArgs);
         pParams[ nArgs ].vt = VT_BSTR | VT_BYREF;
         pParams[ nArgs++ ].pbstrVal = &(varResult.bstrVal);
      ...
      case CMN_CODELISTHOTTRACK:
      {
         nDispID = DISPID_NTFY_CODELISTHOTTRACK;
         LPCM_CODELISTHOTTRACKDATA pild = (LPCM_CODELISTHOTTRACKDATA)pnmhdr;
...
LRESULT CEditX::FireControlEvent(int nEvent, LPNMHDR pnmhdr)
{
   ...
   if (IsActiveXEvent(nEvent))
      ...
      pCPC->FindConnectionPoint(IID_ICodeSenseEvents, &pCP);
      if (!pCP)
         return FALSE;
      ...
      while (pEnum->Next(1, &cd, NULL) == S_OK)
      {
         if (cd.pUnk)
         {
            CComQIPtr<IDispatch, &IID_ICodeSenseEvents> pSink(cd.pUnk);
            if (pSink)
            {
               int nDispID;
               ...
               HRESULT hR = pSink->Invoke(nDispID,
                           IID_NULL,
                           LOCALE_USER_DEFAULT,
                           DISPATCH_METHOD,
                           &disp,
               ...
               if( SUCCEEDED( hR ) )
               {
                  ...
                  case VT_I4:
                     ...
                     break;
                  case VT_BSTR:
                     // NOTE: Be sure to free the memory allocated here!
                     if( varResult.bstrVal )
                        lResult = (LRESULT)BSTR2TSTR( varResult.bstrVal );
                     break;
                  default:
...
{
   CATCH_ERROR_NO_HWND();
   *pbAnswer = (CM_CanUndo(m_hWnd) != 0) ? VARIANT_TRUE : VARIANT_FALSE;
   return S_OK;
}
...
{
   CATCH_ERROR_NO_HWND();
   *pbAnswer = (CM_CanRedo(m_hWnd) != 0) ? VARIANT_TRUE : VARIANT_FALSE;
   return S_OK;
}
...
{
   CATCH_ERROR_NO_HWND();
   *pbAnswer = (CM_CanCut(m_hWnd) != 0) ? VARIANT_TRUE : VARIANT_FALSE;
   return S_OK;
}
...
{
   CATCH_ERROR_NO_HWND();
   *pbAnswer = (CM_CanCopy(m_hWnd) != 0) ? VARIANT_TRUE : VARIANT_FALSE;
   return S_OK;
}
...
{
   CATCH_ERROR_NO_HWND();
   *pbAnswer = (CM_CanPaste(m_hWnd) != 0) ? VARIANT_TRUE : VARIANT_FALSE;
   return S_OK;
}
...
{
   CATCH_ERROR_NO_HWND();
   *pbAnswer = (CM_IsRecordingMacro(m_hWnd) != 0) ? VARIANT_TRUE : VARIANT_FALSE;
   return S_OK;
}
...
{
   CATCH_ERROR_NO_HWND();
   *pbAnswer = (CM_IsPlayingMacro(m_hWnd) != 0) ? VARIANT_TRUE : VARIANT_FALSE;
   return S_OK;
}
...
HRESULT STDMETHODCALLTYPE CEditX::SetFontStyle(cmFontStyleItem lFontStyleItem, cmFontStyle byFontStyle)
{
   ...
   {
      case cmStyText:               cmFontStyles.byText = (BYTE)byFontStyle;               break;
      case cmStyNumber:            cmFontStyles.byNumber = (BYTE)byFontStyle;            break;
      case cmStyKeyword:            cmFontStyles.byKeyword = (BYTE)byFontStyle;            break;
      case cmStyOperator:            cmFontStyles.byOperator = (BYTE)byFontStyle;            break;
      case cmStyScopeKeyword:         cmFontStyles.byScopeKeyword = (BYTE)byFontStyle;         break;
      case cmStyComment:            cmFontStyles.byComment = (BYTE)byFontStyle;            break;
      case cmStyString:            cmFontStyles.byString = (BYTE)byFontStyle;            break;
      case cmStyTagText:            cmFontStyles.byTagText = (BYTE)byFontStyle;            break;
      case cmStyTagEntity:         cmFontStyles.byTagEntity = (BYTE)byFontStyle;            break;
      case cmStyTagElementName:      cmFontStyles.byTagElementName = (BYTE)byFontStyle;      break;
      case cmStyTagAttributeName:      cmFontStyles.byTagAttributeName = (BYTE)byFontStyle;      break;
      case cmStyLineNumber:         cmFontStyles.byLineNumber = (BYTE)byFontStyle;         break;
      default:
      {
         return S_FALSE;
      }
...
HRESULT STDMETHODCALLTYPE CEditX::put_StealKeys( VARIANT_BOOL vBool )
{
   CATCH_ERROR_NO_HWND();
   if (FireOnRequestEdit(DISPID_CMPROP_STEALKEYS) == S_FALSE)
   {
      return S_FALSE;
   }
   m_bStealKeys = (vBool != 0);
   OnChanged(DISPID_CMPROP_STEALKEYS);
   return S_OK;
}

HRESULT STDMETHODCALLTYPE CEditX::get_StealKeys( VARIANT_BOOL __RPC_FAR *pBool )
{
   CATCH_ERROR_NO_HWND();
   *pBool = (VARIANT_BOOL) (m_bStealKeys ? -1 : 0);
   return S_OK;
}
...
STDMETHODIMP CEditX::UpdateControlPositions()
{
   CATCH_ERROR_NO_HWND();
   CM_UpdateControlPositions( m_hWnd );
   return S_OK;
}
