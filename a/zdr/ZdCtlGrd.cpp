...
UINT CGridCtrl::OnGetDlgCode()
{
    ...
    if (m_bHandleTabKey && !mIsCTRL_Pressed())
        nCode |= DLGC_WANTTAB;
...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
   ...
   BOOL bVertScrollAction = FALSE;
...
#ifndef GRIDCONTROL_NO_CLIPBOARD
    if (mIsSHIFT_Pressed() &&(nChar == VK_INSERT))
        OnEditPaste();
#endif
    ...
    else if (nChar == VK_RIGHT || (nChar == VK_TAB && !mIsSHIFT_Pressed()) )
    {
    ...
    else if (nChar == VK_LEFT || (nChar == VK_TAB && mIsSHIFT_Pressed()) )
    {
        // don't let user go to a hidden column
        bFoundVisible = FALSE;
        iOrig = next.col;
    ...
    if (next != m_idCurrentCell)
    {
        ...
        if (m_MouseMode == MOUSE_NOTHING)
        {
            ...
            if (!mIsSHIFT_Pressed() || nChar == VK_TAB)
...
{
    // EFW - BUG FIX
    if (!mIsCTRL_Pressed() && m_MouseMode == MOUSE_NOTHING && nChar != VK_ESCAPE)
    {
...
{
    // EFW - BUG FIX
    if (!mIsCTRL_Pressed() && m_MouseMode == MOUSE_NOTHING && wCharCode != VK_ESCAPE)
...
void CGridCtrl::OnMouseMove(UINT /*nFlags*/, CPoint point)
{
    ...
    if (m_MouseMode != MOUSE_DRAGGING && rect.PtInRect(point) == FALSE )
        return;
#endif
...
// C Maunder     - 29 Aug 2000 - operator= checks for 0 font before setting (Martin Richter)
...
// By default this uses the selected font (which is a bigger font)
CSize CGridCellBase::GetTextExtent(LPCTSTR szText, CDC* pDC /*= 0*/)
{
    CGridCtrl* pGrid = GetGrid();
    ASSERT(pGrid);
...
#ifndef _WIN32_WCE
HCURSOR CGridURLCell::g_hLinkCursor = 0;
#endif
...
void CGridURLCell::OnClick(CPoint PointCellRelative)
{
    ...
    if (GetAutoLaunchUrl() && OverURL(PointCellRelative, strURL))
      ShellExecute(0, _T("open"), strURL, 0,0, SW_SHOW);
#endif
}
...
IMPLEMENT_DYNAMIC( ZGrid, CGridCtrl )

BEGIN_MESSAGE_MAP( ZGrid, CGridCtrl )
   ON_WM_CREATE( )
   ON_WM_SETFOCUS( )
   ON_WM_KILLFOCUS( )
...
// m_pfnCallBack = 0;
   ...
   if ( pCtrlDef->Subtype & zGRID_SCOPE_OI )
   ...
   if ( m_pzsVName )
   {
      ...
      if ( ulListLth )
      {
         ...
         pchListInfo = new char[ ulListLth ];
         GetVariableFromAttribute( pchListInfo, &ulListLth, zTYPE_BLOB,
         ...
         for ( k = 1; k < nCols; k++ )
         {
            ...
            m_pCol[ k ].pchText = new char[ zstrlen( pch ) + 1 ];
            strcpy_s( m_pCol[ k ].pchText, zstrlen( pch ) + 1, pch );

            if ( pchComma )
            {
               *pchComma = '\t';
               m_pCol[ k ].pchMB_Text = new char[ zstrlen( pchComma + 1 ) + 1 ];
               strcpy_s( m_pCol[ k ].pchMB_Text, zstrlen( pchComma + 1 ) + 1, pchComma + 1 );
            }
            else
            ...
            ASSERT( lChar >= 0 );
            pch += sizeof( zLONG );
            m_pCol[ k ].pchTag = new char[ zstrlen( pch ) + 1 ];
            strcpy_s( m_pCol[ k ].pchTag, zstrlen( pch ) + 1, pch );

            // Skip past tag.
            pch += lChar;
            ASSERT( (pch - pchListInfo) <= (zLONG) ulListLth );
         }

         // Loop through m_pchListInfo to see what type of cells should
         // be created for the attributes
         pch = pchListInfo + sizeof( zLONG ) + sizeof( zULONG );
         for ( k = 1; k < nCols; k++ )
         {
//          TraceLineI( "ZSS::MapFromOI col ", k );
            ...
            {
               pch += sizeof( zLONG );
               m_pCol[ k ].pchEntity = new char[ m_pzsEName->GetLength( ) + 1 ];
               strcpy_s( m_pCol[ k ].pchEntity, m_pzsEName->GetLength( ) + 1, *m_pzsEName );
            }
            ...
            if ( (m_pCol[ k ].ulStyle & 0xFFFF0000L) == zGRID_CHECKBOX )
            {
               zLONG lLth = 0;
...
//x}
   ...
   TRY
   {
   // SetRowCount( m_nRows );  later in MapFromOI
      SetColumnCount( nCols );
      SetFixedRowCount( 1 );
      SetFixedColumnCount( 1 );
   }
   ...
   if ( mIs_hWnd( m_hWnd ) ||
        (m_pWndParent && mIs_hWnd( m_pWndParent->m_hWnd ) &&
         Create( CRect( Attr.X, Attr.Y, Attr.X + Attr.W, Attr.Y + Attr.H ),
                 ...
                 Attr.Style | WS_CHILD | WS_BORDER | WS_TABSTOP | WS_VISIBLE )) )
...
zSHORT
ZGrid::FormatTextAtPosition( CString& csText, zLONG lRow, zLONG lCol )
{
   ...
   {
      TraceLineS( "ZListCtrl::FormatTextAtPosition lbAttribute: ",
                  *(m_pZSubtask->m_pzsWndTag) );
      TraceLineS( "ZListCtrl::FormatTextAtPosition Attrib: ",
                  m_pCol[ 0 ].pchAttrib );
   }
#endif
                       ...
                       m_pZSubtask->m_vDialog, zLEVEL_SUBTASK ) <= 0 )
   {
      ...
      // SfLockView( m_vAppList );  // we lock this view because it is
         ...
         m_lEntityNbr = GetRelativeEntityNumber( m_vApp, *m_pzsEName,
                                                 *m_pzsScope, m_ulFlag );
         SetEntityCursor( m_vAppList, *m_pzsEName, 0,
                          zPOS_FIRST | zPOS_RELATIVE | m_ulFlag,
                          0, 0, 0, m_lEntityNbr, *m_pzsScope, 0 );
      }
   }
   ...
   if ( m_lEntityNbr != lRow )
   {
      ...
      // TraceLineI( "ZListCtrl::FormatText DisplaySelectSet: ",
      // m_nDisplaySelSet );
      }
      else
      {
         nPrevSelSet = SetSelectSetForView( m_vAppList, m_nSelectSelSet );
      // TraceLineI( "ZListCtrl::FormatText SelectSelectSet: ",
      //             m_nSelectSelSet );
      }
      ...
      if ( m_lEntityNbr == -1 || (lRow - m_lEntityNbr) != 1 )
      {
         ...
         {
         // TraceLineI( "ZListCtrl::FormatText REL_FIRST failed: ",
         //             lRow );
      ...
      else
      {
         ...
         if ( lRow > 0 )
         {
            ...
            {
            // TraceLineI( "ZListCtrl::FormatText REL_NEXT failed: ",
            //             lRow );
               return( -1 );
            }
         }
   ...
   if ( m_pCol[ lCol ].pchAttrib == 0 || m_pCol[ lCol ].pchAttrib[ 0 ] == 0 )
      nRC = zVAR_0;
   else
   {
      nRC =
...
// if ( zstrcmp( *m_pzsTag, "lbAttribute" ) == 0 )
   ...
   if ( m_pColorList && m_pzsColorAttribute )
   {
      DWORD dw = GetItemData( lRow, lCol );
      if ( dw == 0 )
      {
         zCHAR szValue[ 256 ];
         ...
         ZDualCorrelatedItem *pDualItem = m_pColorList->Find( 0, szValue );
         if ( pDualItem == 0 )
            pDualItem = m_pColorList->Find( szValue );

         if ( pDualItem )
         {
            SetItemData( lRow, lCol, (DWORD) pDualItem );
         }
         else
         {
            SetItemData( lRow, lCol, (DWORD) -1 );
         }
      }
   }
...
GridCallback( GV_DISPINFO *pDispInfo, LPARAM lParam )
{
   ...
   if ( pDispInfo->item.row == 0 || pDispInfo->item.col == 0 )
   {
      ...
      {
         pDispInfo->item.strText = pGrid->m_pCol[ pDispInfo->item.col ].pchText;
      }
      else
      {
   ...
   {
      pDispInfo->item.strText.Format( _T( "Default %d,%d" ),
                                      pDispInfo->item.row, pDispInfo->item.col );
      pGrid->FormatTextAtPosition( pDispInfo->item.strText,
                                   pDispInfo->item.row, pDispInfo->item.col );
   }
...
#if 0
void
ZGrid::MapRowFromOI( zLONG lRow, zLONG lCol, CString& cs )
{
#ifdef DEBUG_ALL
// if ( lRow == 0 )
   {
      TraceLine( "ZGrid::MapRowFromOI: %s  Row: %d  Col: %d",
                 *m_pzsTag, lRow, lCol );
      if ( mIs_hWnd( m_hWnd ) )
      {
         if ( mIs_hWndEnabled( m_hWnd ) )
            TraceLineS( "ZGrid::MapRowsFromOI ", "Enabled" );
         else
            TraceLineS( "ZGrid::MapRowsFromOI ", "Disabled" );
      }
      else
         TraceLineS( "ZGrid::MapRowsFromOI ", "Invalid" );
   }
#endif

   zVIEW  vApp = 0;
   if ( m_pCols && GetViewByName( &vApp, m_csViewName,
                                  m_pZSubtask->m_vDialog,
                                  zLEVEL_SUBTASK ) > 0 )
   {
...
///// code to run thru each CtrlMap entity
   zVIEW  vApp = 0;
   if ( m_pCols && GetViewByName( &vApp, *m_pzsVName,
                                  m_pZSubtask->m_vDialog,
                                  zLEVEL_ANY ) > 0 )
      if ( m_vAppList == 0 )
      {
      ...
      if ( CtrlMapDef.VN[ 0 ] )
      {
         ...
         if ( CtrlMapDef.AN[ 0 ] == 0 )
            TraceLineS( "ZGrid::MapRows View Name with no attribute!!!", "" );
      }
...
// if ( lRow == 0 )
   {
      TraceLineS( "ZGrid::MapRowsFromOI (mid): ", *m_pzsTag );
      if ( mIs_hWnd( m_hWnd ) )
      {
         if ( mIs_hWndEnabled( m_hWnd ) )
            TraceLineS( "ZGrid::MapRowsFromOI ", "Enabled" );
      ...
      {
//       TraceLineI( "ZGrid::MapFromOI row ", lRow );
         ...
         for ( k = 1; k <= lCols; k++ )
         {
//          TraceLineI( "ZGrid::MapFromOI col ", k );
            if ( *pch )
            {
            ...
            // for each column.
            // Sizeable borders do not make sense for a grid.
            ...
            {
//             if ( (ulStyle & 0xFFFF0000L) == zGRID_CHECKBOX )
...
//                                                      zUSE_DEFAULT_CONTEXT );
               ...
               case zGRID_COMBOBOX:
//                TraceLineI( "Grid combobox ", ulStyle );
...
//                                                0 : zUSE_DEFAULT_CONTEXT );
                  ...
                  if ( (ulSubtype & zCONTROL_DISABLED) == 0 )
                  {
                     if ( DrAllocTaskMemory( (zCOREMEM) &pchBuffer,
                                             zMAX_BLOB ) != 0 )
                     {
                        IssueError( m_pZSubtask->m_vDialog, 0, 0,
                                    "Memory alloc error - Combo SS" );
                        break;
                     }
...
//                                                    zUSE_DEFAULT_CONTEXT );
               ...
               default:
                  TraceLineI( "Invalid Grid style ", ulStyle );
            }
         }
...
// if ( lRow == 0 )
   {
      TraceLineS( "ZGrid::MapRowsFromOI (post): ", *m_pzsTag );
      if ( mIs_hWnd( m_hWnd ) )
      {
         if ( mIs_hWndEnabled( m_hWnd ) )
            TraceLineS( "ZGrid::MapRowsFromOI ", "Enabled" );
...
// else
//    TraceLineS( "ZGrid::MapFromOI m_vAppList null ", *m_pzsTag );
...
//?for ( lCol = 1; lCol <= m_nVisibleRows; lCol++ )
//?   Clear( SS_ALLCOLS, ssCol );

   if ( mIs_hWnd( m_hWnd ) &&
        m_pCol && GetViewByName( &vApp, *m_pzsVName,
                                 m_pZSubtask->m_vDialog,
                                 zLEVEL_ANY ) > 0 )
   {
#ifdef DEBUG_ALL
...
//x   zBOOL bPrevRedraw = SetBool( SSB_REDRAW, FALSE );
   ...
   // SfLockView( m_vAppList );  // we lock this view because it is
      ...
      SetRowCount( m_lEntityCnt + 1 );  // VSetMax( m_lEntityCnt );
      ...
      for ( k = 1; k <= m_nRows; k++ )
      {
         for ( j = 1; j <= m_nCols; j++ )
         {
            SetItemState( k, j, GetItemState( k, j ) & ~GVIS_READONLY );
         }
      }
#endif

   //?GetTopLeftPrev( &ssCol, &lRowPrev );
   //?GetTopLeftCell( &ssCol, &lRow );
//    TraceLineI( "SS lRowPrev = ", lRowPrev );
//    TraceLineI( "SS lRowCurr = ", lRow );

#ifdef DEBUG_ALL
      TraceLineS( "ZGrid::MapFromOI (mid1): ", *m_pzsTag );
      if ( mIs_hWnd( m_hWnd ) )
      {
         if ( mIs_hWndEnabled( m_hWnd ) )
            TraceLineS( "ZGrid::MapFromOI ", "Enabled" );
      ...
      // current grid viewing range and data-fill the corresponding
      // cells in the grid.  In the following tests, lRow will be
      ...
      // display row.
      if ( lRow > 0 && lRowPrev > 0 )
      {
         zLONG k = lRow - lRowPrev;
         if ( k == 1 || k == -1 )
         {
            ...
            if ( k == 1 )
            {
               lRow = lRowPrev + m_nVisibleRows;
            }
            else
            {
               // leave lRow as it was
            }
         }
         else
         {
...
#ifdef DEBUG_ALL
      TraceLineS( "ZGrid::MapFromOI (mid2): ", *m_pzsTag );
      if ( mIs_hWnd( m_hWnd ) )
      {
         if ( mIs_hWndEnabled( m_hWnd ) )
            TraceLineS( "ZGrid::MapFromOI ", "Enabled" );
      ...
      TraceLineI( "...lRow (position within oi) = ", lRow );
      TraceLineI( "...SS display rows = ", m_nVisibleRows );
#endif

//    MapRowsFromOI( lRow, lRowCnt, nDisplayRows );
  //? MapRowsFromOI( lRow, lRowCnt, m_nVisibleRows ); // force all rows to Map

#ifdef DEBUG_ALL
      TraceLineS( "ZGrid::MapFromOI (mid3): ", *m_pzsTag );
      if ( mIs_hWnd( m_hWnd ) )
      {
         if ( mIs_hWndEnabled( m_hWnd ) )
            TraceLineS( "ZGrid::MapFromOI ", "Enabled" );
...
//    if ( CheckExistenceOfEntity( vApp, *m_pzsEName ) == zCURSOR_SET )
                            ...
                            0, 0, 0, 0, cpcScope, 0 ) >= 0 )
      {
         ...
         SetEntityCursor( vApp, *m_pzsEName, 0,
                          ...
                          0, 0, 0, 0, cpcScope, 0 );
         lRowCnt = GetRelativeEntityNumber( vApp, *m_pzsEName,
                                            cpcScope, 0L );

#ifdef DEBUG_ALL
      TraceLineS( "ZGrid::MapFromOI (mid4): ", *m_pzsTag );
      DisplayObjectInstance( vApp, *m_pzsEName, 0 );
#endif

//???    if ( GetActiveCell( &ssCol, &lRow ) == FALSE )
//???       ssCol = 1;
//???
...
//       ShowActiveCell( SS_SHOW_CENTERLEFT );
      ...
      {
#ifdef DEBUG_ALL
         TraceLineS( "ZGrid::MapFromOI (mid5): ", *m_pzsTag );
...
//    TraceLineX( "Invalidating SS hWnd: ", (zLONG) m_hWnd );
   ...
   {
#ifdef DEBUG_ALL
      TraceLineS( "ZGrid::MapFromOI (mid6): ", *m_pzsTag );
#endif
...
#ifdef DEBUG_ALL
   TraceLineS( "ZGrid::MapFromOI (post): ", *m_pzsTag );
   if ( mIs_hWnd( m_hWnd ) )
   {
      if ( mIs_hWndEnabled( m_hWnd ) )
         TraceLineS( "ZGrid::MapFromOI ", "Enabled" );
...
zSHORT
ZGrid::MapRowToOI( zLONG lFlag, zLONG lRow )
{
#ifdef DEBUG_ALL
   if ( lRow == 1 )
   {
      TraceLineS( "ZGrid::MapRowToOI: ", *m_pzsTag );
      if ( mIs_hWnd( m_hWnd ) )
      {
         if ( mIs_hWndEnabled( m_hWnd ) )
            TraceLineS( "ZGrid::MapRowToOI ", "Enabled" );
         else
            TraceLineS( "ZGrid::MapRowToOI ", "Disabled" );
      }
      else
         TraceLineS( "ZGrid::MapRowToOI ", "Invalid" );
   }
#endif
   ...
   // do not do mapping.
   if ( m_pCol && (vApp = IsVisibleForMapToOI( m_csViewName )) != 0 )
   {
      zCPCHAR  cpcScope;
      zPCHAR   pchOn;
      zPCHAR   pchOff;
      ...
      zLONG    lActiveRow;    // Row and column for the currently
      zLONG    lActiveCol;    // active cell
      zSHORT nRC = 0;
   ...
   //?if ( GetActiveCell( &lActiveCol, &lActiveRow ) == 0 ||
   //?     lActiveRow == 0 || lActiveCol == 0 )
      {
         return( 0 ); // cannot retrieve the active cell or not in data cell
      }

      if ( lActiveCol > lColCnt )
         lActiveCol = lColCnt;

//    TraceLineI( "Current row: ", lActiveRow );
//    TraceLineI( "Current col: ", lActiveCol );
...
//    TraceLineI( "ZGrid::MapToOI row ", lActiveRow );
      ...
      nRC = SetEntityCursor( m_vAppList, *m_pzsEName, 0,
                             zPOS_FIRST | zPOS_RELATIVE, 0, 0, 0,
                             lActiveRow - 1, cpcScope, 0 );
      if ( nRC > zCURSOR_UNCHANGED )
      {
         nRC = 0;
         for ( k = 1; k <= m_nCols; k++ )
         {
//          TraceLineI( "ZGrid::MapToOI col ", k );

            ulStyle = m_pCol[ k ].ulStyle;
            ulSubtype = m_pCol[ k ].ulSubtype;
            if ( (ulSubtype & zCONTROL_DISABLED) ||
                 lActiveCol != -1 && k != lActiveCol )
            {
            ...
            switch ( ulStyle & 0xFFFF0000L )
            {
               ...
               // TraceLineS( szMsg, "" );
                  ...
                  if ( CheckExistenceOfEntity( m_vAppList,
                                    m_pCol[ k ].pchEntity ) == zCURSOR_SET &&
                       MiGetUpdateForViewEntityAttr( m_vAppList,
                                                     m_pCol[ k ].pchEntity,
                                                     m_pCol[ k ].pchAttrib,
                                                     0 ) > 0 )
...
{
   int nRC = CGridCtrl::OnCreate( lpCreateStruct );
   if ( nRC == -1 )
      return( -1 );
...
void
ZGrid::OnSysKeyDown( UINT uKey, UINT uRepeatCnt, UINT uFlags )
{
   CGridCtrl::OnSysKeyDown( uKey, uRepeatCnt, uFlags );
}

void
ZGrid::OnKeyDown( UINT uKey, UINT uRepeatCnt, UINT uFlags )
{
   CGridCtrl::OnKeyDown( uKey, uRepeatCnt, uFlags );
}

void
ZGrid::OnKeyUp( UINT uKey, UINT uRepeatCnt, UINT uFlags )
{
   CGridCtrl::OnKeyUp( uKey, uRepeatCnt, uFlags );
}

void
ZGrid::OnChar( UINT uKey, UINT uRepeatCnt, UINT uFlags )
{
   CGridCtrl::OnChar( uKey, uRepeatCnt, uFlags );
}

void
ZGrid::OnLButtonDown( UINT uModKeys, CPoint pt )
{
   CGridCtrl::OnLButtonDown( uModKeys, pt );
}

void
ZGrid::OnLButtonUp( UINT uModKeys, CPoint pt )
{
   CGridCtrl::OnLButtonUp( uModKeys, pt );
}

void
ZGrid::OnLButtonDblClk( UINT uModKeys, CPoint pt )
{
   CGridCtrl::OnLButtonDblClk( uModKeys, pt );
}

void
ZGrid::OnMouseMove( UINT uModKeys, CPoint pt )
{
   CGridCtrl::OnMouseMove( uModKeys, pt );
}
...
ZGrid::OnSetFocus( CWnd *pWndLostFocus )
{
   if ( m_pZSubtask->InformDriverOfSetFocus( this ) )
      CGridCtrl::OnSetFocus( pWndLostFocus );
}
...
ZGrid::OnKillFocus( CWnd *pWndGetFocus )
{
   ...
   CGridCtrl::OnKillFocus( pWndGetFocus );
}

void
ZGrid::OnHScroll( UINT nSBCode, UINT nPos, CScrollBar *pScrollBar )
{
   CGridCtrl::OnHScroll( nSBCode, nPos, pScrollBar );
}

void
ZGrid::OnVScroll( UINT nSBCode, UINT nPos, CScrollBar *pScrollBar )
{
   CGridCtrl::OnVScroll( nSBCode, nPos, pScrollBar );
}

void
ZGrid::OnSize( UINT nType, int cx, int cy )
{
   CGridCtrl::OnSize( nType, cx, cy );
}

void
ZGrid::OnTimer( UINT nIDEvent )
{
...
{
   return( CGridCtrl::OnEraseBkgnd( pDC ) );
}
...
BOOL
ZGrid::ValidateEdit( int nRow, int nCol, LPCTSTR cpcText )
{
   CGridCellBase *pCell = GetCell( nRow, nCol );
   ASSERT( pCell );
   if ( !pCell )
...
zSHORT
ZGrid::SetRowColText( zCPCHAR cpcText, zLONG lRow, zLONG lColumn )
{
   if ( lColumn <= m_nCols && lColumn > 0 )
   {
      ...
      if ( lRow == 0 )
      {
      //?SetColumnTitle( (zPCHAR) cpcText, (zSHORT) lColumn );
      }
      else
      {
      //?SetData( (zSHORT) lColumn, lRow, (zPCHAR) cpcText );
      }
   }
                      ...
                      zLONG lRow, zLONG lColumn )
{
   if ( lColumn <= m_nCols && lColumn > 0 )
   {
      ...
      if ( lRow == 0 )
      {
      //?GetColumnTitle( pchText, (zSHORT) lColumn );
      }
      else
      {
      //?GetData( lColumn, lRow, pchText );
      }
   }
...
//./ ADD NAME=GridLockColumn
// Source Module=zdctlo.cpp
/////////////////////////////////////////////////////////////////////////////
//
//  DIALOG OPERATION: GridLockColumn
               ...
               zSHORT   bLock )
{
...
#ifdef DEBUG_ALL
   TraceLineI( "In GridLockColumn ", (zLONG) vSubtask );
#endif
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for GridLockColumn ", cpcCtrlTag );
   }

   return( -1 );
}

//./ ADD NAME=GridGetActiveCell
// Source Module=zdctlo.cpp
/////////////////////////////////////////////////////////////////////////////
//
//  DIALOG OPERATION: GridGetActiveCell
//                                              05/27/93 Modified: 05/27/93
//
...
//./ END + 5
zOPER_EXPORT zSHORT OPERATION
GridGetActiveCell( zVIEW    vSubtask,
                   zCPCHAR  cpcCtrlTag,
                   zPLONG   plActiveRow,
                   zPLONG   plActiveCol )
{
...
#ifdef DEBUG_ALL
   TraceLineI( "In GridGetActiveCell ", (zLONG) vSubtask );
#endif
   ...
   *plActiveRow = *plActiveCol = 0;
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      if ( pGrid )
      {
      //?if ( pGrid->GetActiveCell( plActiveCol, plActiveRow ) == 0 ||
      ...
      TraceLineS( "drvr - Invalid control type for GridGetActiveCell ", cpcCtrlTag );
   }

   return( -1 );
}

//./ ADD NAME=GridGetCellData
// Source Module=zdctlo.cpp
/////////////////////////////////////////////////////////////////////////////
//
//  DIALOG OPERATION: GridGetCellData
//                                              05/27/93 Modified: 05/27/93
//
...
//./ END + 6
zOPER_EXPORT zSHORT OPERATION
GridGetCellData( zVIEW   vSubtask,
                 zCPCHAR cpcCtrlTag,
                 ...
                 zLONG   lColNbr )
{
...
#ifdef DEBUG_ALL
   TraceLineI( "In GridGetCellData ", (zLONG) vSubtask );
#endif
   ...
   *pchReturnData = 0;
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ZGrid *pGrid = DYNAMIC_DOWNCAST( ZGrid, pzma->m_pCtrl );
      if ( pGrid )
         return( pGrid->GetData( lColNbr, lRowNbr, pchReturnData ) );

      TraceLineS( "drvr - Invalid control type for GridGetCellData ", cpcCtrlTag );
   }

   return( -1 );
}

//./ ADD NAME=GridSetCellData
// Source Module=zdctlo.cpp
/////////////////////////////////////////////////////////////////////////////
//
//  DIALOG OPERATION: GridSetCellData
//                                              05/27/93 Modified: 05/27/93
//
//  PURPOSE:    This operation gets the data in grid cell (row/column).
//
...
//./ END + 6
zOPER_EXPORT zSHORT OPERATION
GridSetCellData( zVIEW    vSubtask,
                zCPCHAR  cpcCtrlTag,
                zPCHAR   pchData,
                zLONG    lRowNbr,
                zLONG    lColNbr )
{
...
#ifdef DEBUG_ALL
   TraceLineI( "In GridSetCellData ", (zLONG) vSubtask );
#endif
   ...
   //?   return( !pSS->SetData( lColNbr, lRowNbr, pchData ) );
      ...
      TraceLineS( "drvr - Invalid control type for GridSetCellData ",
                  cpcCtrlTag );
   }
