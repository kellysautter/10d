...
// OVERVIEW
// ========
// Source file for implementation of general purpose debugging utilities.
...
//    Made MsgList available outside of zdrapp.dll.
//
...
// #endif ... this whole thing is under the debug umbrella
...
//
#if 0
int ZTraceFn::nIndent=-1;      // current indent level

void
AFXAPI AfxDump( const CObject *pOb )
{
   afxDump << pOb;
}

void
AFX_CDECL AfxTrace( LPCTSTR lpszFormat, ... )
{
   ...
   ASSERT( nBuf < mElementCnt( szBuffer ) );

   // PD: Here are my added lines to do the indenting. Search
   // for newlines and insert prefix before each one. Yawn.
   //
   static zBOOL bStartNewLine = TRUE;
   zPCHAR nextline;
   for ( zPCHAR start = szBuffer; *start; start = nextline + 1 )
   {
      if ( bStartNewLine )
      {
         if ( (afxTraceFlags & traceMultiApp) && (AfxGetApp( ) != 0) )
            afxDump << AfxGetApp( )->m_pszExeName << ": ";

         afxDump << CString( ' ', ZTraceFn::nIndent );
         bStartNewLine = FALSE;
      }
...
LPCTSTR ZWndMsgMap::Names[ WM_USER ];  // name of each WM_ message
...
//
CString
ZWndMsgMap::GetMsgName( UINT uMsg )
{
   static CString csName;
   if ( uMsg >= WM_USER )
      csName.Format( "WM_USER+%d", uMsg - WM_USER );
   else
   if ( Names[ uMsg ] )
      csName = Names[ uMsg ];
   else
      csName.Format( "0x%04x", uMsg );
...
//
CString
DbgWndName( UINT uMsg )
{
   static ZWndMsgMap wndMsgMap; // instantiate 1st time called
...
//
DBGINTERFACENAME *_pDbgInterfaceNames = 0;

/////////////////////////////////////////////////////////////////////////////
// Get OLE interface name.
//
CString
DbgWndName( REFIID iid )
{
   ...
   static CString cs;
   cs.Format( "{%08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x}",
