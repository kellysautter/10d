...
// Sqlite is almost exactly the same as MySQL so they share much of the same code.
   ...
   #define MAX_TABLENAME_LTH     32 // Changing from 34 (dks) because 10c has 32 and we currently need to be able to switch back and forth between the two.
   #define MAX_COLUMNNAME_LTH    32 // Changing from 34 (dks) because 10c has 32 and we currently need to be able to switch back and forth between the two.
   ...
   switch ( pchDataType[ 0 ] )
   {
      ...
      case zTYPE_INTEGER:
		  if (CompareAttributeToString(vDTE, "TE_FieldDataRel", "SQL_NULLS", "Y") == 0)
		  {
			  ...
			  // KJS 04/18/25 - At HFI, we are using AutoIncrement and so the database is creating the key. We want the statement to be:
			  // int4 GENERATED BY DEFAULT AS IDENTITY( INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 START 1 CACHE 1 NO CYCLE) NOT NULL
			  // I guess this would not be the case if not AutoIncrement. But at the moment I'm not sure the difference.
				  ...
				  vDTE, "TE_FieldDataRel", "ZKey", "TE_TablRec") >= 0)
			  {
				  if (CheckExistenceOfEntity(vDTE, "ER_Entity") >= zCURSOR_SET)
					  ...
					  if (CheckExistenceOfEntity(vDTE, "ER_EntIdentifier") >= zCURSOR_SET &&
						  ...
						  strcat_s(pchEnd, lEndRemainingLth, " int4 GENERATED BY DEFAULT AS IDENTITY( INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 START 1 CACHE 1 NO CYCLE)");
					  else
						  sprintf_s(pchEnd, lEndRemainingLth, " INTEGER");
				  else
					  sprintf_s(pchEnd, lEndRemainingLth, " INTEGER");
	  ...
	  case 'A':
		  ...
		  if (pchKeyType[0] == 'D')
		  {
			  ...
			  sprintf_s(pchEnd, lEndRemainingLth, " SERIAL PRIMARY KEY ");
		  }
		  else
		  {
			  // This must be a FK so don't declare it as SERIAL/KEY.
	  ...
	  case 'G':
		  ...
		  sprintf_s(pchEnd, lEndRemainingLth, " BIGINT");
		  break;

		  // UUID
	  case 'U':
		  //zsprintf(pchEnd, " UUID");
		  sprintf_s(pchEnd, lEndRemainingLth, " UUID");
		  break;

	  case 'O':
		  //zsprintf(pchEnd, " BOOLEAN");
		  sprintf_s(pchEnd, lEndRemainingLth, " BOOLEAN");
		  break;
          ...
          zVIEW  vSubtask )
{
   ...
   if ( fnWriteLine( vDTE, f, szLine ) < 0 )
      goto EndOfFunction;
       ...
       // KJS 04/18/25 - We now have a schema name. Use that if it exists.
	   GetAddrForAttribute( &pchSchemaName, vDTE, "TE_DBMS_Source", "Name" );
	   if ( pchSchemaName[0] == 0 )
          sprintf_s( szLine, zsizeof( szLine ), "SET SCHEMA '%s' %s", pchDatabaseName, LINE_TERMINATOR );
	   else
		   sprintf_s(szLine, zsizeof(szLine), "SET SCHEMA '%s' %s", pchSchemaName, LINE_TERMINATOR);
