...
zOPER_EXPORT char OPERATION operator <= ( zSHORT n, ZDecimal zDec )
{
   ZDecimal zDecN;
   zDecN = n;
   return( zDecN <= zDec );
...
zOPER_EXPORT char OPERATION operator != ( ZDecimal zDec, zCHAR n )
{
   ZDecimal zDecN;
   zDecN = (zSHORT) n;
   return( zDec != zDecN );
...
ARACrypt::SetKey( zCPCHAR cpcKey )
{
   ...
   if ( m_szKey[ 0 ] == 0 )
   {
      ...
      strcpy_s( szSeed, zsizeof( szSeed ), "\x43\xC8\x21\xD3\xF4\xB3\x10\x27\x09\xAA\x18\x56" );
...
{
   strcpy_s( pchKey, lKeyLth, m_szKey );
}
...
unsigned
char  EncryptA[] = { 22, 31,  5, 13, 26, 16, 15, 28,  0,  6,
                     23,  2, 19, 12,  9, 10, 30,  1, 25,  4,
                     29, 17,  7, 18, 27, 11, 21, 14, 24, 20, 3, 8 };
unsigned
char  EncryptB[] = { 10,  9, 17,  4, 31, 22,  1,  6, 29, 20,
...
//./ END + 2
zSHORT
fnEncryptStringUL( zPCHAR pchOut, zPCHAR pchIn, zPCHAR pchKey, zUSHORT uLth, zBOOL bEncodeLth )
{
   ARACrypt crypt;
   zPCHAR   pch;
   zUCHAR   uCrypt;
...
// TraceLineS( szMsg, "" );
   ...
   if ( pchKey && *pchKey )
   {
      ...
      if ( lKeyLth > 0 )
      {
         strncpy_s( szNewKey + lInLth, zsizeof( szNewKey ) - lInLth, pchKey, lKeyLth );
   ...
   if ( uLth > zMAXEL || (bEncodeLth && lInLth >= uLth) || (bEncodeLth == FALSE && lInLth > uLth) )
   {
...
// srand( (unsigned) time( NULL ) );
   ...
   srand( (unsigned) *((unsigned *) ((char *) pch + k)) ); // set a random starting point
...
{
   return( fnEncryptStringUL( pchOut, pchIn, pchKey, uLth, TRUE ) );
}

//./ ADD NAME=UfEncryptStringLong
// Source Module=kzoedeca.cpp
/////////////////////////////////////////////////////////////////////////////
//
// ENTRY:      UfEncryptStringLong
//
// PARAMETERS: pchOut - return string encrypted
//             pchIn  - String to encrypt
//             pchKey - Encryption key (may be null)
...
// DESCRIPTION - This utility takes in a string and returns an
//               encrypted version of the string with a length
//               of the passed length value.
//               The maximum length was zMAXEL ... that limit has
//               been relaxed (2007.01.01).
//
// RETURNS:  0 - Success
//
/////////////////////////////////////////////////////////////////////////////
...
UfEncryptStringLong( zPCHAR pchOut, zPCHAR pchIn, zPCHAR pchKey, zUSHORT uLth )
{
   ...
   while ( nRC == 0 && lInLth > 0 )
   {
      // The maximum password length is zMAXEL - 1.
      if ( lInLth > zMAXEL )
      {
         uCurrLth = zMAXEL;
         ch = pchIn[ nPos + zMAXEL ];
         pchIn[ nPos + zMAXEL ] = 0;
      }
      else
      {
         if ( nPos == 0 ) // first time in
            uCurrLth = uLth;
         else
            uCurrLth = lInLth;
      ...
      nRC = fnEncryptStringUL( pchOut + nPos, pchIn + nPos, pchKey, uCurrLth, FALSE );
      if ( ch )
         pchIn[ nPos + zMAXEL ] = ch;
...
//./ END + 2
zSHORT OPERATION
fnDecryptStringUL( zPCHAR pchOut, zPCHAR pchIn, zPCHAR pchKey, zUSHORT uLth, zBOOL bEncodeLth )
{
   ARACrypt crypt;
   zUCHAR   uCrypt;
   zUSHORT  uChar;
...
// zsprintf( szMsg, "UfDecryptStringUL Length: %d   In: \"%s\"", uLth, pchIn );
// TraceLineS( szMsg, "" );
   ...
   if ( bEncodeLth )
   {
      ...
      if ( uLth > zMAXEL ||
           (bEncodeLth && lInLth >= uLth) ||
           (bEncodeLth == FALSE && lInLth > uLth) ||
           lInLth < 0 )
      {
...
{
   return( fnDecryptStringUL( pchOut, pchIn, pchKey, uLth, TRUE ) );
}
...
UfDecryptStringLong( zPCHAR pchOut, zPCHAR pchIn, zPCHAR pchKey, zUSHORT uLth )
{
   ...
   while ( nRC >= 0 && lInLth > 0 )
   {
      // The maximum password length is zMAXEL.
      if ( lInLth > zMAXEL )
      {
         uCurrLth = zMAXEL;
         ch = pchIn[ nPos + zMAXEL ];
      ...
      nRC = fnDecryptStringUL( pchOut + nPos, pchIn + nPos, pchKey, uCurrLth, FALSE );
      if ( ch )
         pchIn[ nPos + zMAXEL ] = ch;
...
// pfn = fnDecimalOperation;   // in kzoesyaa.c
   ...
   if ( nRC == zCALL_ERROR )
   {
      TraceLineS( "(ZDecimal) Error calling INIT function in ", "zDecimalOperation" );
   }
   ...
   {
      char szValue[ zMAX_FILENAME_LTH + 1 ];
      ...
      if ( lpLibrary == 0 )
      {
         ...
         strupr_s( szValue, zsizeof( szValue ) );

         if ( szValue[ 0 ] != 'Y' && szValue[ 0 ] != '1' && szValue[ 0 ] != 'T' )
         {
